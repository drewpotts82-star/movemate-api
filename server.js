...
// ── HEALTH CHECK ──────────────────────────────────────────────────────────────
app.get('/', (req, res) => {
  res.json({ status: 'MoveMate API running', version: '2.1.0', model: 'sms-email-blast' });
});
...
function isAusMobile(phone) {
  ...
  const cleaned = phone.replace(/\s/g, '');
  // Australian mobiles start with 04 or +614
...
async function sendEmail(to, subject, body) {
  // TODO: plug in Resend/SendGrid when ready
  console.log(`[EMAIL] To: ${to} | Subject: ${subject} | ${body.substring(0, 80)}`);
  return true;
}
  ...
  try {
    ...
    for (const biz of matchingBiz) {
      if (isAusMobile(biz.phone)) {
        ...
        // Landline or no phone — send email instead
...
app.post('/partners/register', async (req, res) => {
  try {
    ...
    if (isAusMobile(phone)) {
      ...
      const { subject, body } = {
        subject: `Welcome to MoveMate, ${business_name}!`,
...
app.listen(PORT, () => {
  console.log(`MoveMate API v2.1 on port ${PORT}`);
...
Done
Yes — select all and replace the entire contents of server.js in GitHub with the code above. Here's exactly how:

Go to your GitHub repo → click server.js
...
Added isAusMobile() function — checks if number starts with 04 or +614
Lead blast now routes: mobile → SMS, landline/no phone → email, neither → skipped + logged
Partner registration also routes the welcome message the same way
sendEmail() is wired up but just logs for now — once you pick an email service (Resend is the easiest, free tier) I'll plug it in with one function swap
Response now returns both businessesNotified and emailsNotified
Once committed, Railway will auto-redeploy. Then drop the bond cleaning CSV here when it's done! 🚀
